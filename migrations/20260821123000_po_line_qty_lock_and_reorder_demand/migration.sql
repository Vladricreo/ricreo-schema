-- Blocco qty buyer + domanda riordino senza doppio conteggio assembly/PO
-- e consumo materiali in unità dello SKU preferito.

ALTER TABLE inventory."PurchaseOrderLine"
  ADD COLUMN IF NOT EXISTS "quantityLocked" BOOLEAN NOT NULL DEFAULT false;
...
bucket_sums AS (
    SELECT
        spec_id,
        SUM(CASE WHEN days_ago >= 0 AND days_ago < 8 THEN quantity ELSE 0 END)::NUMERIC AS s0,
        SUM(CASE WHEN days_ago >= 8 AND days_ago < 31 THEN quantity ELSE 0 END)::NUMERIC AS s1,
        SUM(CASE WHEN days_ago >= 31 THEN quantity ELSE 0 END)::NUMERIC AS s2,
        COUNT(*)::INT AS movement_count,
        CEIL(SUM(quantity))::BIGINT AS total_consumption,
...
-- Stesso allineamento sulla view per Item (orfani / pagina consumi):
-- C/P/U da ProductOrder solo se non collegati a un AssemblyOrder.
...
LEFT JOIN production_demand_by_item pd ON pd.item_id = i.id
