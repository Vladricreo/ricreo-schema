-- Labor/pricing view fix: evita attribuzioni cross-operatore e usa fallback conservativo con pochi campioni.
-- Sync da prisma/migrations/sql/product_cost_views.sql.
...
-- dello stesso operatore/tipo il cui startedAt cade in [sessione inizio, sessione fine].
...
-- IQR (Tukey 1.5), min 10s/unita, cap 6h/unita; poi winsor P5-P95 per gruppo (code alta e bassa:
...
operator_hourly_cost AS (
    SELECT
        ws."userId" AS operator_id,
        (
            SUM(ws.cost::NUMERIC * (ws."endMinute" - ws."startMinute")::NUMERIC)
            / NULLIF(SUM((ws."endMinute" - ws."startMinute")::NUMERIC), 0)
        ) AS avg_hourly_rate
    FROM public."WorkShift" ws
    WHERE ws."isActive" = TRUE
      AND ws.cost IS NOT NULL
      AND ws."endMinute" > ws."startMinute"
    GROUP BY ws."userId"
),
tab_sessions AS (
    SELECT
...
session_qty_by_type AS (
    SELECT
        ...
        MAX(
            COALESCE(
                matched_shift.cost::NUMERIC,
                ohc.avg_hourly_rate,
                (SELECT hourly_rate FROM settings_labor),
                0
            )
        ) AS hourly_rate
    ...
    INNER JOIN inventory."AssemblyOperation" sq
       ...
       AND sq."operatorId" = ts.operator_id
       AND sq."type" = ts.operation_type
    ...
    LEFT JOIN LATERAL (
        SELECT ws.cost
        FROM public."WorkShift" ws
        WHERE ws."userId" = ts.operator_id
          AND ws."isActive" = TRUE
          AND ws.cost IS NOT NULL
          AND ws."dayOfWeek" = EXTRACT(DOW FROM ts.session_started_at)::INT
          AND (
              EXTRACT(HOUR FROM ts.session_started_at)::INT * 60
              + EXTRACT(MINUTE FROM ts.session_started_at)::INT
          ) >= ws."startMinute"
          AND (
              EXTRACT(HOUR FROM ts.session_started_at)::INT * 60
              + EXTRACT(MINUTE FROM ts.session_started_at)::INT
          ) < ws."endMinute"
        ORDER BY ws."startMinute" DESC
        LIMIT 1
    ) matched_shift ON TRUE
    LEFT JOIN operator_hourly_cost ohc ON ohc.operator_id = ts.operator_id
    GROUP BY
        ts.session_id,
        ts.product_id,
        ts.operator_id,
...
raw_ops AS (
    SELECT
        ...
        (
            (LEAST(sbt.session_duration_sec, st.total_qty * 21600) / NULLIF(st.total_qty, 0))
            / 3600.0
            * COALESCE(sbt.hourly_rate, 0)
...
stage_avg_total AS (
    SELECT
        ...
        AVG(labor_cost_per_unit) AS avg_stage_labor_cost
    FROM robust_ops
    GROUP BY product_id, operation_type
),
stage_monthly_detail AS (
    SELECT
        ...
        AVG(labor_cost_per_unit) AS month_avg_labor_cost
    FROM robust_ops
...
stage_avg_monthly AS (
    SELECT
        ...
        AVG(month_avg_labor_cost) AS avg_stage_labor_cost_monthly,
...
stage_median AS (
    SELECT
        ...
        PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY labor_cost_per_unit) AS median_stage_labor_cost
    FROM robust_ops
    GROUP BY product_id, operation_type
),
product_totals AS (
    SELECT
        ...
        COALESCE(SUM(t.avg_stage_labor_cost), 0)::NUMERIC AS avg_labor_cost_per_unit_total,
        COALESCE(SUM(m.median_stage_labor_cost), 0)::NUMERIC AS median_labor_cost_per_unit,
        COALESCE(SUM(mo.avg_stage_labor_cost_monthly), SUM(t.avg_stage_labor_cost), 0)::NUMERIC AS avg_labor_cost_per_unit_monthly
...
recent_product_totals AS (
    SELECT
        ...
        COALESCE(SUM(t.avg_stage_labor_cost), 0)::NUMERIC AS recent_avg_labor_cost_per_unit,
        COALESCE(SUM(m.median_stage_labor_cost), 0)::NUMERIC AS recent_median_labor_cost_per_unit
...
SELECT
    ...
    CAST(pt.median_labor_cost_per_unit AS DECIMAL(12,4)) AS median_labor_cost_per_unit,
    CAST(pt.avg_labor_cost_per_unit_total AS DECIMAL(12,4)) AS avg_labor_cost_per_unit_total,
    CAST(pt.avg_labor_cost_per_unit_monthly AS DECIMAL(12,4)) AS avg_labor_cost_per_unit_monthly,
    CASE
        WHEN COALESCE(rsc.recent_sample_size, 0) >= 3 THEN CAST(rpt.recent_median_labor_cost_per_unit AS DECIMAL(12,4))
        ELSE NULL
    END AS recent_median_labor_cost_per_unit,
    CASE
        WHEN COALESCE(rsc.recent_sample_size, 0) >= 3 THEN CAST(rpt.recent_avg_labor_cost_per_unit AS DECIMAL(12,4))
        ELSE NULL
    END AS recent_avg_labor_cost_per_unit
...
-- Le quantita vengono attribuite solo a sessioni dello stesso operatore e dello stesso tipo.
-- ============================================================================
...
per_stage AS (
    SELECT
        ...
        AVG(labor_cost_per_unit) AS avg_labor_cost,
        PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY labor_cost_per_unit) AS median_labor_cost,
...
SELECT
    ...
    CAST(COALESCE(SUM(median_labor_cost), 0) AS DECIMAL(12,4)) AS median_labor_cost_per_unit,
    CAST(COALESCE(SUM(avg_labor_cost), 0) AS DECIMAL(12,4)) AS avg_labor_cost_per_unit
FROM per_stage
...
-- Non include: costo lavoro (quello e operativo).
-- ============================================================================
...
SELECT
    ...
    -- Costo operativo (lavoro): costo/pezzo pesato per operatore e fallback al tempo base con pochi campioni.
    ...
    -- Secondi/unita usati per il costo operativo (mix graduale recente/storico)
...
CROSS JOIN LATERAL (
    SELECT
        CASE
            WHEN lt.product_id IS NULL OR COALESCE(lt.sample_size_total, 0) < 3 THEN
                COALESCE((SELECT base_minutes FROM settings_base_labor), 1)::NUMERIC * 60.0
