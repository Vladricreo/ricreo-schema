-- ============================================================================
-- P&L: quantity 0 non è una vendita (FBM Amazon annullati).
--
-- Il report all-orders FBM lascia spesso item-status vuoto e quantity 0.
-- Prima is_sold restava true (stato vuoto / Unshipped) e le matview
-- mv_sales_analytics_daily / mv_overview_* prendevano spedizione FBM e
-- commissioni intere su righe senza pezzi: profitto negativo finto.
--
-- Stesse colonne della vista precedente: CREATE OR REPLACE è sufficiente.
-- Dopo il deploy: REFRESH di mv_sales_analytics_* e mv_overview_*.
-- ============================================================================
...
FROM (
  SELECT
    ...
    -- Pending/unshipped restano vendite (ultimi giorni Amazon), ma quantity 0
    -- è un annullamento FBM: niente ricavo né costi provvisori.
    (
      o.quantity > 0
      AND (
