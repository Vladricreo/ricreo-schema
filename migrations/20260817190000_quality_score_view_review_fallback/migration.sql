-- La view agganciava le recensioni solo su ASIN + marketplace.
-- I riassunti ci sono quasi solo su IT: gli altri paesi restavano NULL
-- anche con recensioni sullo stesso ASIN. Fallback sull'ultimo riassunto ASIN.
...
latest_reviews_for_store AS (
  SELECT DISTINCT ON (upper("asin"), "storeKey")
  ...
  WHERE "channel" = 'AMAZON'
    ...
    AND length(trim("asin")) > 0
  ORDER BY upper("asin"), "storeKey", "capturedAt" DESC
),
latest_reviews_for_asin AS (
  ...
  WHERE "channel" = 'AMAZON'
    AND "asin" IS NOT NULL
    AND length(trim("asin")) > 0
  ORDER BY upper("asin"), "capturedAt" DESC
),
bullet_stats AS (
  SELECT
    c."sku",
...
checks AS (
  SELECT
    ...
    CASE
      WHEN COALESCE(r."count", ra."count") IS NULL THEN NULL
      ELSE COALESCE(r."count", ra."count") >= 20
    END AS reviews_twenty_plus,
    CASE
      WHEN COALESCE(r."rating", ra."rating") IS NULL THEN NULL
      ELSE COALESCE(r."rating", ra."rating") >= 4
    END AS reviews_four_plus
  FROM latest_content c
  JOIN bullet_stats b ON b."sku" = c."sku" AND b."storeKey" = c."storeKey"
  LEFT JOIN latest_reviews_for_store r
    ON r.asin_key = upper(c."asin") AND r."storeKey" = c."storeKey"
  LEFT JOIN latest_reviews_for_asin ra
    ON ra.asin_key = upper(c."asin")
)
SELECT
