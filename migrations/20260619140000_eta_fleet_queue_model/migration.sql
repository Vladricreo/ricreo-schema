-- ============================================================================
-- FIX SMART ETA — modello a "coda di flotta condivisa"
-- Problema: senza copertura Gantt, ogni job calcolava l'ETA partendo da "adesso"
--   e dividendo solo per le PROPRIE stampanti => molti job finivano lo stesso
--   giorno, ignorando che la flotta (N stampanti) e condivisa tra tutti i job.
-- Soluzione: la flotta e una coda condivisa. L'ETA di un job =
--   (lavoro cumulato "davanti" a lui, in ordine priorita -> FIFO) / N_stampanti.
--   Il "lavoro effettivo" include il tempo di harvest per piatto (gap di scarico,
--   che occupa la stampante). La banda nasce dalla contesa per classe di priorita
--   e dalla variabilita dell'harvest, sempre divisa per la flotta reale.
...
-- Totali di lavoro per classe di priorita (per la contesa ottimistica/pessimistica)
prio_tot AS (
  SELECT priority, SUM(work_minutes) AS w, SUM(plates_remaining) AS p
  FROM job_work GROUP BY priority
),
-- Cumulato "atteso" in ordine priorita DESC -> FIFO (createdAt, number)
ordered AS (
  SELECT jw.*,
...
calc AS (
  SELECT
    ...
    -- lavoro/piatti delle classi a priorita strettamente maggiore (davanti per certo)
    ...
    -- totale della stessa classe di priorita (incl. self)
...
final AS (
  SELECT c.*,
    ...
    -- stampanti che il singolo job puo' usare (per il floor "job da solo")
...
SELECT
  ...
  -- ETA OTTIMISTICA: solo classi a priorita maggiore davanti (le pari-priorita
  -- girano in parallelo), harvest minimo, divisa per la flotta; con floor
  -- "job da solo su max stampanti".
  ...
  -- ETA ATTESA: cumulato in ordine priorita -> FIFO, harvest medio, / flotta.
  CASE WHEN f.scheduled_end IS NOT NULL THEN
  ...
  END AS "etaExpected",
  -- ETA PESSIMISTICA: tutta la classe di pari priorita (e superiori) davanti,
  -- harvest massimo (ore notturne), / flotta.
  CASE WHEN f.scheduled_end IS NOT NULL THEN
